  ...
  def Rope.run
    ...
    while iline = fin.gets
      ...
      while step < steps
        ...
        # check if tail needs adjustment
        ...
        if xdiff > 1 || (need_diag && xdiff > 0)
          puts "Moving right because #{xdiff > 1 ? 'not adjacent' : 'diag required'}" if debug
          any_move = true
          tail[0] = tail[0] + 1
        elsif xdiff < -1 || (need_diag && xdiff == -1)
          puts "Moving left because #{xdiff < -1 ? 'not adjacent' : 'diag required'}" if debug
          any_move = true
          tail[0] = tail[0] - 1
        end
        ...
        if ydiff > 1 || (need_diag && ydiff > 0)
          any_move = true
          puts "Moving down because #{ydiff > 1 ? 'not adjacent' : 'diag required'}" if debug
          tail[1] = tail[1] + 1
        elsif ydiff < -1 || (need_diag && ydiff == -1)
          puts "Moving up because #{ydiff < -1 ? 'not adjacent' : 'diag required'}" if debug
          tail[1] = tail[1] - 1
          any_move = true
        end

        puts "Not moving the tail as we're adjacent, overlapping or touching diagonally" if !any_move && debug

        # for some reason I have to dup...

        pretty(no,ohead,otail,head,tail,dirs[dir],covered_positions)
    ...
    puts "Tail covered #{covered_positions.size} unique positions."
